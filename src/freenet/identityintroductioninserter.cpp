	...
	if(message["Identifier"].find("IdentityIntroductionInserter")==0)
	{
		...
		if(message.GetName()=="PutFailed")
		{
			...
			if(message["Fatal"]=="true" || message["Code"]=="9")
			{
				...
				m_log->WriteLog(LogFile::LOGLEVEL_WARNING,__FUNCTION__" received fatal error trying to insert IdentityIntroduction "+idparts[3]);
			}
			m_inserting=false;
			return true;
		}
		...
		if(message.GetName()=="PutSuccessful")
		{
			...
			m_inserting=false;
			m_log->WriteLog(LogFile::LOGLEVEL_INFO,__FUNCTION__" successfully inserted IdentityIntroduction "+idparts[3]);
			return true;
		}
...
void IdentityIntroductionInserter::StartInsert(const long localidentityid, const std::string &day, const std::string &UUID, const std::string &solution)
{
	...
	{
		m_log->WriteLog(LogFile::LOGLEVEL_DEBUG,__FUNCTION__" could not find a public key for identity.  It is probably a new identity that doesn't have a key yet.");
	}
